...
# ── 便捷接口：直接从 results dict 计算 CHAIR ──

def evaluate_from_results(results: dict, coco_anno_dir: str, cache_path: str = None):
    """从我们的 results dict 直接计算 CHAIR，不需要中间 JSONL 文件

    Args:
        results: {"B1_Direct": [{image, answer, ...}, ...], ...}
        coco_anno_dir: COCO 标注目录（含 captions_val2017.json 和 instances_val2017.json）
        cache_path: 缓存 evaluator 的 pickle 路径（可选）

    Returns:
        {"B1_Direct": {"CHAIRs": ..., "CHAIRi": ..., "Recall": ..., "Len": ...}, ...}
    """
    ...
    for name, items in results.items():
        ...
        for r in items:
            json.dump({"image_id": r.get("image", ""), "caption": r.get("answer", "")}, tmp)
            tmp.write("\n")
        tmp.close()

        # 计算
        cap_dict = evaluator.compute_chair(tmp.name, "image_id", "caption")
        all_metrics[name] = cap_dict["overall_metrics"]
        os.unlink(tmp.name)

    return all_metrics


def print_baseline_comparison(all_metrics: dict):
    """打印 baseline 对比表"""
    print(f"{'Baseline':<20s} {'CHAIRs%':>8s} {'CHAIRi%':>8s} {'Recall%':>8s} {'Len':>6s}")
    print("-" * 55)
    for name, m in all_metrics.items():
        print(f"{name:<20s} {m['CHAIRs']*100:>7.1f} {m['CHAIRi']*100:>7.1f} {m['Recall']*100:>7.1f} {m['Len']*100:>5.1f}")